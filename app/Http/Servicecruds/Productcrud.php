...
class Productcrud
{
    ...
    public function productData( $request)
    {
        ...
        if(!empty($products))
        {
            foreach ($products as $key=>$product)
            {
                ...
                if($product->unit_id)
                    $nestedData['unit'] = $product->unit->unit_code;
                else
                    $nestedData['unit'] = 'N/A';
    ...
    public function productscreate()
    {
            ...
            $lims_unit_list = Unit::where('status', true)->get();
            ...
            return view('superadmin.product.create',compact('lims_product_list_without_variant', 'lims_product_list_with_variant', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_warehouse_list', 'numberOfProduct', 'custom_fields'));
    ...
    public function autoPurchase($product_data, $warehouse_id, $stock)
    {
        ...
        if($product_data->tax_id) {
            ...
            if($product_data->tax_method == 1) {
                $net_unit_cost = number_format($product_data->product_cost, 2, '.', '');
                $tax = number_format($product_data->product_cost * $stock * ($tax_data->rate / 100), 2, '.', '');
                $cost = number_format(($product_data->product_cost * $stock) + $tax, 2, '.', '');
            }
            else {
                $net_unit_cost = number_format((100 / (100 + $tax_data->rate)) * $product_data->product_cost, 2, '.', '');
                $tax = number_format(($product_data->product_cost - $net_unit_cost) * $stock, 2, '.', '');
                $cost = number_format($product_data->product_cost * $stock, 2, '.', '');
            }
            ...
            $data['total_tax'] = 0.00;
            $data['total_cost'] = number_format($product_data->product_cost * $stock, 2, '.', '');
            $net_unit_cost = number_format($product_data->product_cost, 2, '.', '');
            $tax_rate = 0.00;
            $tax = 0.00;
            $cost = number_format($product_data->product_cost * $stock, 2, '.', '');
        ...
        $product_warehouse_data = ProductWarehouse::select('id', 'qty')
                                ->where([
    ...
    public function history( $request)
    {
        ...
        if($role->hasPermissionTo('product_history')) {
            ...
            $product_data = Product::select('product_name', 'product_code')->find($product_id);
            $lims_warehouse_list = Warehouse::where('is_active', true)->get();
            return view('superadmin.product.history',compact('starting_date', 'ending_date', 'warehouse_id', 'product_id', 'product_data', 'lims_warehouse_list'));
        }
        else
        ...
        if(Auth::user()->role_id > 2) {
            return ProductVariant::join('variants', 'product_variants.variant_id', '=', 'variants.id')
                ->join('product_warehouses', function($join) {
                    $join->on('product_variants.product_id', '=', 'product_warehouses.product_id');
                    $join->on('product_variants.variant_id', '=', 'product_warehouses.variant_id');
                })
                ->select('variants.name', 'product_variants.item_code', 'product_variants.additional_cost', 'product_variants.additional_price', 'product_warehouses.qty')
                ->where([
                    ['product_warehouses.product_id', $id],
                    ['product_warehouses.warehouse_id', Auth::user()->warehouse_id]
                ])
    ...
    public function productsedit($id)
    {
            ...
            $noOfVariantValue = 0;
            $custom_fields = CustomField::where('belongs_to', 'product')->get();
            return view('superadmin.product.edit',compact('lims_product_list_without_variant', 'lims_product_list_with_variant', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_product_data', 'lims_product_variant_data', 'lims_warehouse_list', 'noOfVariantValue', 'custom_fields'));
    ...
    public function getData($id, $variant_id)
    {
        if($variant_id) {
            $data = Product::join('product_variants', 'products.id', 'product_variants.product_id')
                ->select('products.product_name', 'product_variants.item_code')
                ->where([
                    ['products.id', $id],
            ...
            $data = Product::select('product_name', 'product_code')->find($id);
        return $data;
    }
    ...
    public function productWarehouseData($id)
    {
        ...
        if($lims_product_data->is_variant) {
            $lims_product_variant_warehouse_data = ProductWarehouse::where('product_id', $lims_product_data->id)->orderBy('warehouse_id')->get();
            $lims_product_warehouse_data = ProductWarehouse::select('warehouse_id', DB::raw('sum(qty) as qty'))->where('product_id', $id)->groupBy('warehouse_id')->get();
            ...
            $lims_product_warehouse_data = ProductWarehouse::where('product_id', $id)->orderBy('warehouse_id', 'asc')->get();
        }
    ...
    //     return view('superadmin.product.print_barcode', compact('lims_product_list_without_variant', 'lims_product_list_with_variant', 'preLoadedproduct'));
    // }
    ...
    public function importProduct( $request)
    {
        ...
        while($columns=fgetcsv($file))
        {
           ...
           $lims_category_data = Category::firstOrCreate(['name_en' => $data['category'], 'is_active' => true]);
           ...
           $product = Product::firstOrNew([ 'product_name'=>$data['product_name'], 'is_active'=>true ]);
            if($data['product_image'])
                $product->product_image = $data['product_image'];
            else
                $product->product_image = 'zummXD2dvAtI.png';

           $product->product_name = htmlspecialchars(trim($data['product_name']));
           $product->product_code = $data['product_code'];
           $product->product_type = strtolower($data['product_type']);
           ...
           $product->product_cost = str_replace(",","",$data['product_cost']);
           $product->product_price = str_replace(",","",$data['product_price']);
           ...
           if($data['variantvalue'] && $data['variant_name']) {
                ...
                $variant_names = explode(",", $data['variant_name']);
                ...
                foreach ($variant_names as $key => $variant_name) {
                    ...
                    if($data['itemcode'])
                        ...
                        $item_code = $variant_name . '-' . $data['product_code'];
    ...
    public function deleteBySelection( $request)
    {
        ...
        foreach ($product_id as $id) {
            ...
            if($lims_product_data->product_image) {
                $images = explode(",", $lims_product_data->product_image);
