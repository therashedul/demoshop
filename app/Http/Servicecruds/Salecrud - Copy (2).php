...
class Salecrud
{
    public function saleindex($request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($request->filled('warehouse_id')) {
                $sales = $customsales->where('warehouse_id', $request->warehouse_id);
            }  
            ...
            if ($request->filled('sale_name')) {
                $sales = $customsales->where('reference_no', 'like', '%' . $request->sale_name . '%');
            }
    ...
    public function saleedit($id)
    {
            ...
            return view('superadmin.sale.edit',compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list', 'lims_sale_data','lims_product_sale_data', 'currency_exchange_rate'));
    ...
    public function saleupdate($request, $id)
    {
        ...
        foreach ($product_id as $key => $pro_id) {
            ...
            if($sale_unit[$key] != 'n/a') {
                ...
                if($data['sale_status'] == 1) {
                    ...
                    if($lims_product_data->is_variant) {
                        ...
                        $lims_product_warehouse_data = ProductWarehouse::FindProductWithVariant($pro_id, optional($lims_product_variant_data)->variant_id, $data['warehouse_id'])
                        ->first();

                        $product_sale['variant_id'] = optional($lims_product_variant_data)->variant_id;
                        optional($lims_product_variant_data)->qty -= $new_product_qty;
                        optional($lims_product_variant_data)->save();
                    }
                    ...
                    optional($lims_product_warehouse_data)->qty -= $new_product_qty;
                    $lims_product_data->save();
                    optional($lims_product_warehouse_data)->save();
                }
            }
            else
                $sale_unit_id = 0;
            ...
            if(isset($product_variant_id[$key])){
        ...
        //collecting mail data
        // $mail_setting = Mail::latest()->first();
        if($lims_customer_data->email) {
            $mail_data['email'] = $lims_customer_data->email;
            $mail_data['reference_no'] = $lims_sale_data->reference_no;
            $mail_data['sale_status'] = $lims_sale_data->sale_status;
