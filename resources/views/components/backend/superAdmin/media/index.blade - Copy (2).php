...
@section('content')
                ...
                <div class="card-body">
                    ...
                    <div id="third" style=" display: none;">
                        <div class="jumbotron">
                            ...
                            <form id="dropzoneForm" class="dropzone" action="{{ route('admin.media.upload') }}">
                                @csrf
                    ...
                    @foreach ($data as $key => $imagefile)
                        @if ($imagefile->extention == '.pdf')
                            <div class="col-sm-6 col-md-2 col-lg-2 mb-3">
                                ...
                                <input type="hidden" id="selected_img_id_{{ $imagefile->id }}"
                                    value="{{ $imagefile->id }}">
                                <a id="btn_delete_post_main_image_{{ $imagefile->id }}"
                                    onclick="myFunction_{{ $imagefile->id }}()"
                        ...
                        @elseif ($imagefile->extention == '.docx')
                            <div class="col-sm-6 col-md-2 col-lg-2 mb-3">
                                <form action="{{ route('admin.media.delete') }}" method="get">
                                    <input type="hidden" name="name" id="selected_img_name_{{ $imagefile->name }}"
                                        value="{{ $imagefile->name }}">
                                    <input type="hidden" name="id" id="selected_img_id_{{ $imagefile->id }}"
                                        value="{{ $imagefile->id }}">
                                    <button class="btn btn-danger btn_img_delete btn-sm">
                                        <i class="fa fa-times"></i>
                                    </button>
                                </form>
                                {{-- onclick="myFunction_{{ $imagefile->id }}()" --}}
                        ...
                        <script type="text/javascript">
                            $(document).ready(function() {
                                $("body").on("click", "#btn_delete_post_main_image_{{ $imagefile->id }}", function(e) {
                                    $.ajaxSetup({
                                        headers: {
                                    ...
                                    // alert('ok');
                                    e.preventDefault();
                                    var file_name = document.getElementById("selected_img_name_{{ $imagefile->name }}")
                                        .value;
    ...
    <script type="text/javascript">
        Dropzone.options.dropzoneForm = {
            ...
            dictDefaultMessage: "Drop image and file here to upload",
