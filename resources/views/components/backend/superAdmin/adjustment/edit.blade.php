            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex align-items-center">
                        <h4>{{trans('Update Adjustment')}}</h4>
                    </div>
                    <div class="card-body">
                        <p class="italic"><small>{{trans('The field labels marked with * are required input fields')}}.</small></p>
                        {!! Form::open(['route' => ['superAdmin.qty_adjustment.update', ['id'=>$limsadjustmentdata->id]], 'method' => 'put', 'files' => true, 'id' => 'adjustment-form']) !!}
                        <div class="row">
                            <div class="col-md-12">
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <p><strong>{{$limsadjustmentdata->reference_no}}</strong></p>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input type="hidden" name="warehouse_id_hidden" value="{{$limsadjustmentdata->warehouse_id}}">
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <div class="table-responsive mt-3">
                                            <table id="myTable" class="table table-hover order-list">
                                                ...
                                                <tbody>
                                                	@foreach($limsproductadjustmentdata as $product_adjustment_data)
                                                	<tr>
                                                	@php
                                                	...
                                                	<td class="action">
                                                		<select name="action[]" class="form-control act-val">
                                                			...
                                                			<option value="+">{{trans("Addition")}}</option>
                                                			<option value="-">{{trans("Subtraction")}}</option>
                                                			@else
                                                			<option value="-">{{trans("Subtraction")}}</option><option value="+">{{trans("Addition")}}</option>
                                                			@endif
                                                		</select>
                                                	</td>
                                                	<td><button type="button" class="ibtnDel btn btn-md btn-danger">{{trans("delete")}}</button>
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <textarea rows="5" class="form-control" name="note">{{$limsadjustmentdata->note}}</textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
...
for(rowindex  =0; rowindex <= rownumber; rowindex++){
    ...
    let url = "{{ route('superAdmin.adjustment.getproduct', ':id') }}";
    let unUrl = url.replace(':id', id);
    $.get(unUrl, function(data) {
        lims_product_array = [];
        product_code = data[0];
        product_name = data[1];
        product_qty = data[2];
    ...
    function productSearch(data){
        var codeUrl = "{{ route('superAdmin.product_adjustment.search') }}";
        $.ajax({
            type: 'GET',
            url: codeUrl,
            data: {
            ...
            success: function(data) {
                ...
                if(flag){
                    ...
                    cols += '<td class="action"><select name="action[]" class="form-control act-val"><option value="-">{{trans("Subtraction")}}</option><option value="+">{{trans("Addition")}}</option></select></td>';
