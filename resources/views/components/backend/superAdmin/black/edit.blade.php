...
@section('content')
    <form action="{{ route('superAdmin.black.update') }}" method="post">
        @csrf
        ...
        <div class="x_panel">
            ...
            <div class="x_content">
                    ...
                    <select class="custom-select" name="user_id" id="inputGroupSelect01">
                        ...
                        @if ($users)
                            @foreach ($users as $user)
                                <option {{ $user->id == $black->user_id ? 'selected=""' : '' }} value="{{ $user->id }}">
                                    {{ $user->name }}</option>
                            @endforeach
                        @endif
                    </select>
                ...
                <div class="item form-group">
                    ...
                    <input type="text" name="ip" class="form-control" value="{{ $black->ip }}" placeholder="IP">
                </div>
                <div class="item form-group">
