...
<body>
        ...
        <div id="receipt-data">
            <div class="centered">
                @if($general_setting->site_logo)
                    <img src="{{url('public/logo', $general_setting->site_logo)}}" height="42" width="50" style="margin:10px 0; filter: brightness(1);">
                @endif
            ...
            <table class="table-data">
                <tbody>
                    ...
                    <?php 
                        ...
                        if($product_sale_data->variant_id) {
                            ...
                            $product_name = $lims_product_data->product_name.' ['.$variant_data->variant_name.']';
                        }
                        elseif($product_sale_data->product_batch_id) {
                            $product_batch_data = \App\Models\ProductBatch::select('batch_no')->find($product_sale_data->product_batch_id);
                            $product_name = $lims_product_data->product_name.' ['.trans("file.Batch No").':'.$product_batch_data->batch_no.']';
                        }
                        else
                ...
                <!-- <tfoot> -->
                    ...
                    @endif
                    @php
                    //    $grandtotal = number_format((float)$lims_sale_data->grand_total, 0, '.', '') + number_format((float)$lims_sale_data->order_tax, 2, '.', '')+number_format((float)$lims_sale_data->shipping_cost, 0, '.', '') - number_format((float)$lims_sale_data->coupon_discount, 2, '.', '') - number_format((float)$lims_sale_data->coupon_discount, 2, '.', '')- number_format((float)$lims_sale_data->order_discount, 2, '.', '');
                    @endphp
                    <tr>
                    ...
                    <tr>
                        ...
                        @else
                        @endif --}}
                        <th class="centered" colspan="3">{{trans('file.In Words')}}: <span>{{str_replace("-"," ",$numberInWords)}}</span> </th>
                    </tr>
                </tbody>
                <!-- </tfoot> -->
            </table>
            <table>
                <tbody>
                    ...
                    @endforeach
                    <tr><td class="centered" colspan="3">{{trans('file.Thank you for shopping with us')}}</td></tr>
                    <tr>
    ...
    <script type="text/javascript">
            ...
            setTimeout(auto_print, 10000000);
        });
    </script>
</body>
