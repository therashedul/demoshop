                        ...
                        @foreach ($permissions as $pvalue)
                            ...
                            @if ($permissionId == $pid)
                                @if ($pvalue->name == 'page-create')
                                    <a class="btn btn-primary mb-2 ml-2" href="{{ route('admin.page.create') }}">Add
                                        New Page</a>
                                @endif
                        ...
                        @if ($permissionId == $pid)
                            @if ($pvalue->name == 'page-archive')
                                <span class="float-right">
                                    <a href="{{ route('admin.page.archived') }}"
                ...
                <table class="table table-striped jambo_table bulk_action">
                    ...
                    <tbody>
                        ...
                        @foreach ($userdata as $user)
                            ...
                            @endphp
                        @endforeach
                        <form method="post" action="{{ route('admin.page.multipledelete') }}">
                            {{ csrf_field() }}
                            @foreach ($rhps as $rhpsvalue)
                                @php
                                    $permissionId = $rhpsvalue->permission_id;
                                    ...
                                    <td>
                                        @foreach ($rhps as $rhpsvalue)
                                            ...
                                            @foreach ($permissions as $pvalue)
                                                ...
                                                @endphp
                                                @if ($permissionId == $pid)
                                                    @if ($pvalue->name == 'page-status')
                                                        @if ($value->status == 1)
                                                            <a href="{{ route('admin.page.publish', $value->id) }}"
                                                                class="btn btn-info btn-sm "><i
                                                                    class="fa fa-arrow-circle-up"
                                                                    aria-hidden="true"></i></a>
                                                        @else
                                                            <a href="{{ route('admin.page.unpublish', $value->id) }}"
                                                    ...
                                                    @if ($pvalue->name == 'page-deleted')
                                                        <a href="{{ route('admin.page.delete', $value->id) }}"
