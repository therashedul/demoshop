        ...
        <div class="title_right">
            <div class="col-md-5 col-sm-5   form-group pull-right top_search">
                <div class="input-group">
                    {{-- <a class="btn btn-primary mb-2 ml-2" href="{{ route('admin.post.create') }}">
                        Add New Post
                    </a> --}}
                    @php
                            ...
                            @if ($permissionId == $pid)
                                @if ($pvalue->name == 'post-create')
                                    <span class="float-right">
                                        <a class="btn btn-primary btn-sm" href="{{ route('admin.post.create') }}"><i
                    ...
                    @foreach ($permissions as $pvalue)
                        ...
                        @if ($permissionId == $pid)
                            @if ($pvalue->name == 'post-archive')
                                <span class="float-right">
                                    <a href="{{ route('admin.post.archive') }}"
                ...
                <table class="table table-striped jambo_table bulk_action">
                    ...
                    <tbody>
                        ...
                        @foreach ($categories as $cvalue)
                            ...
                            @endphp
                        @endforeach
                        <form method="post" action="{{ route('admin.post.multipledelete') }}">
                            {{ csrf_field() }}
                            @foreach ($rhps as $rhpsvalue)
                                @php
                                    $permissionId = $rhpsvalue->permission_id;
                            ...
                            @foreach ($posts as $value)
                                <tr>
                                    ...
                                    <td>
                                                            ...
                                                            @if (stristr($name, 'post'))
                                                                ...
                                                                @if ($valuename == 'show')
                                                                    <a class="btn btn-success btn-sm"
                                                                        href="{{ route('admin.post.show', $value->id) }}"><i
                                                                            class="fas fa-eye"></i></a>
                                                                @elseif ($valuename == 'create')
                                                                    {{-- <a class="btn btn-primary"
                                                                    href="{{ route('admin.post.create') }}">New
                                                                    value</a> --}}
                                                                @elseif ($valuename == 'status')
                                                                    @if ($value->status == 1)
                                                                        <a href="{{ route('admin.post.publish', $value->id) }}"
                                                                            class="btn btn-info btn-sm"><i
                                                                                class="fa fa-arrow-circle-up"
                                                                                aria-hidden="true"></i></a>
                                                                    @else
                                                                        <a href="{{ route('admin.post.unpublish', $value->id) }}"
                                                                ...
                                                                @elseif ($valuename == 'edit')
                                                                    <a class="btn btn-primary btn-sm"
                                                                        href="{{ route('admin.post.edit', $value->id) }}"><i
                                                                            class="fas fa-edit"></i></a>
                                                                @elseif ($valuename == 'delete')
                                                                    <a class="btn btn-danger btn-sm"
                                                                        href="{{ route('admin.post.delete', $value->id) }}">
                                                                        <i class="fa fa-trash"></i></a>
                                                                    {{-- {!! Form::open([
                                                                        'method' => 'DELETE',
                                                                        'route' => ['admin.post.destroy', $value->id],
                                        ...
                                        <a href="{{ route('admin.post.edit', $value->id) }}"
                                            class="btn btn-sm btn-primary"><i class="fa fa-pencil-square"
                                                aria-hidden="true"></i></a>
                                        <a href="{{ route('admin.post.delete', $value->id) }}"
