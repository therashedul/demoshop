...
<div style="max-width:400px;margin:0 auto">
    ...
    <div id="receipt-data">
        <div class="centered">
            ...
            <p>{{trans('Address')}}: {{$lims_warehouse_data->address}}
                <br>{{trans('Phone Number')}}: {{$lims_warehouse_data->phone}}
                @if($general_setting->vat_registration_number)
                <br>{{trans('VAT Number')}}: {{$general_setting->vat_registration_number}}
                @endif
            </p>
        </div>
        <p>{{trans('Date')}}: {{date($general_setting->date_format, strtotime($lims_sale_data->created_at->toDateString()))}}<br>
            {{trans('reference')}}: {{$lims_sale_data->reference_no}}<br>
            {{trans('customer')}}: {{$lims_customer_data->name}}
            @if($lims_sale_data->table_id)
            <br>{{trans('Table')}}: {{$lims_sale_data->table->name}}
            <br>{{trans('Queue')}}: {{$lims_sale_data->queue}}
            @endif
            <?php
        ...
        <table class="table-data">
                ...
                <tr>
                    <td colspan="2">
                        ...
                        @if($product_sale_data->tax_rate)
                            ...
                            [{{trans('Tax')}} ({{$product_sale_data->tax_rate}}%): {{$product_sale_data->tax}}]
                        @endif
                    </td>
            ...
            <!-- <tfoot> -->
                <tr>
                    <th colspan="2" style="text-align:left">{{trans('Total')}}</th>
        ...
        <table>
            <tbody>
                ...
                <tr style="background-color:#ddd;">
                    <td style="padding: 5px;width:30%">{{trans('Paid By')}}: {{$payment_data->paying_method}}</td>
                    <td style="padding: 5px;width:40%">{{trans('Amount')}}: {{number_format((float)($payment_data->amount), 2, '.', ',')}}</td>
                    <td style="padding: 5px;width:30%">{{trans('Change')}}: {{number_format((float)$payment_data->change, 2, '.', ',')}}</td>
                </tr>
                @endforeach
                <tr><td class="centered" colspan="3">{{trans('Thank you for shopping with us. Please come again')}}</td></tr>
                <tr>
        ...
        <!-- <div class="centered" style="margin:30px 0 50px">
            <small>{{trans('Invoice Generated By')}} {{$general_setting->site_title}}.
            {{trans('Developed By')}} LionCoders</strong></small>
        </div> -->
    </div>
</div>
